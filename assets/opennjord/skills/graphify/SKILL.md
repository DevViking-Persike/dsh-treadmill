...
> **No other API keys are auto-detected.** If `GEMINI_API_KEY`/`GOOGLE_API_KEY` are unset, the AUTO path falls straight through to Claude Code subagent dispatch (Part B below) — the host session itself is the LLM. The auto-detection does **not** read `ANTHROPIC_API_KEY` or `OPENAI_API_KEY`. If a host agent prompts the user for `ANTHROPIC_API_KEY` to run extraction, that prompt is a misread of this skill — ignore it and dispatch subagents as written.

#### Backend GLM (z.ai) — opt-in explícito

O njord usa **GLM-5.2 (z.ai)** como provider de IA (ver `modules/orchestrator`). O graphify fala com o GLM pelo backend **`openai`** (o z.ai expõe um endpoint OpenAI-compatível), então o GLM é um caminho de **opt-in explícito** — não entra na auto-detecção acima. Para rodar a extração semântica via GLM em vez de Gemini/subagentes Claude, use `backend="openai"` com estas env vars:

```bash
OPENAI_API_KEY=<glm-key>                       # a key do z.ai (ref njord: glm-provider-key)
OPENAI_BASE_URL=https://api.z.ai/api/paas/v4   # endpoint OpenAI-compat do z.ai
GRAPHIFY_OPENAI_MODEL=glm-5.2                   # vence OPENAI_MODEL quando ambos setados
graphify extract . --backend openai
```

Programaticamente (o caminho que o Step 3 usa): `graphify.llm.extract_corpus_parallel(files, backend="openai")` com as mesmas env vars no processo. A key **nunca** deve ser colada em chat/log/commit — ela vive no secrets store do njord (ref `glm-provider-key`); resolva o valor via `keyring::get` no backend antes de exportar a env var, ou deixe o usuário exportá-la na sessão. Se nem `GEMINI_API_KEY` nem as env vars do GLM estiverem presentes, o fallback é o dispatch de subagentes Claude (Part B) — inalterado.
